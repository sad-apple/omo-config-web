...
export function RuntimeFallbackConfigForm({
  ...
  const retryOnErrors = value?.retry_on_errors?.join(", ") ?? DEFAULT_CONFIG.retry_on_errors.join(", ");
  const maxFallbackAttempts = value?.max_fallback_attempts ?? DEFAULT_CONFIG.max_fallback_attempts;
  const cooldownSeconds = value?.cooldown_seconds ?? DEFAULT_CONFIG.cooldown_seconds;
  const timeoutSeconds = value?.timeout_seconds ?? DEFAULT_CONFIG.timeout_seconds;
  const notifyOnFallback = value?.notify_on_fallback ?? DEFAULT_CONFIG.notify_on_fallback;
